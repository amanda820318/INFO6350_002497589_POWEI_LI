...
PhoneSignInManager {
  ...
  Future updateEmail({
    ...
    try {
      ...
      if (e.code == 'requires-recent-login') {
        ScaffoldMessenger.of(context).hideCurrentSnackBar();
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Too long since most recent sign in. Sign in again before updating your email.')),
        );
      }
    }
  }
  ...
  Future<BaseAuthUser?> _signInOrCreateAccount(
    ...
    try {
      final userCredential = await signInFunc();    if (userCredential?.user != null) {
      await maybeCreateUser(userCredential!.user!);
    }
      return userCredential == null
          ? null
          : ToDoList2024WinterFirebaseUser.fromUserCredential(userCredential);
    } on FirebaseAuthException catch (e) {
